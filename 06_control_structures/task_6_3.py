...
for intf, vlans_list in fast_int['trunk'].items():
    print('interface FastEthernet' + intf)
    operator: str = vlans_list.pop(0)
    vlans_list_str: str = ','.join(vlan for vlan in vlans_list)
    for command in trunk_template:
        if command.endswith('allowed vlan'):
            if operator == 'add':
                print(' {} add {}'.format(command, vlans_list_str))
            if operator == 'del':
                print(' {} remove {}'.format(command, vlans_list_str))
            if operator == 'only':
                print(' {} {}'.format(command, vlans_list_str))
        else:
            print(' {}'.format(command))
